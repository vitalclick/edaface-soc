    ...
    <div class="card-body px-3 py-4">
        <form action="<?php echo site_url('blogs'); ?>" method="get" class="w-100">
            <div class="form-floating blog-search-box d-flex">
                <input class="form-control blog-search-input py-4" value="<?php if(isset($search_string) && !empty($search_string)) echo $search_string; ?>" type="text" id="blog_search" placeholder="<?php echo site_phrase('find_your_best_blogs'); ?>" name="search" onkeyup="show_submit_button(this)" onblur="show_submit_button(this)">
                ...
                <a href="<?php echo site_url('blogs?category='.$blog_category['slug']); ?>" class="py-3 px-2 list-group-item-action <?php if(isset($_GET['category']) && $_GET['category'] == $blog_category['slug'])echo 'bg-light'; ?>" aria-current="true">
                    <div class="d-flex w-100 justify-content-between">
                        ...
                        <?php if($popular_category['blog_number'] > 0): ?>
                            <span class="badge badge-primary rounded-pill"><?php echo $popular_category['blog_number']; ?></span>
                        <?php endif; ?>
                    </div>
        ...
        <h6 class="m-0 px-2 pt-4 pb-3 fw-700 border-bottom"><?php echo site_phrase('latest_blogs'); ?></h6>
        <div class="list-group">
            <?php foreach($latest_blogs->result_array() as $latest_blog): ?>
                <a href="<?php echo site_url('blog/details/'.slugify($latest_blog['title']).'/'.$latest_blog['blog_id']); ?>" class="px-2 py-3 bg-transparent list-group-item list-group-item-action">
                    ...
                    <div>
                        ...
                        <p class="mt-2 mb-0 text-muted" style="font-size: 12px;"><?php echo get_past_time($latest_blog['added_date']); ?></p>
                    </div>
                </a>
            <?php endforeach; ?>
